...
//        }
        ...
        LuckUser user = luckUserService.findByUid(uid);
        if(user.getScore()<20){
            return  fail("积分不足");
        }
        LuckProduct dod = luckDog();

        //中奖了
        LuckUserProduct luckUserProduct = new LuckUserProduct();
        luckUserProduct.setUid(uid);
        luckUserProduct.setPid(dod.getId());
        luckUserProduct.setPname(dod.getName());
        luckUserProductService.add(luckUserProduct);

        //扣分
        Integer score = user.getScore();
        score = score-20;
        score = score+dod.getValue();
        luckUserService.updateScoreById(score,uid);

        return success(dod.getName(), "");
    }

    private LuckProduct luckDog(){
        ...
        if(luckProduct!=null&&luckProducts.size()>0){
           for(int i=0;i<luckProducts.size();i++){
               ...
               tt.put(luckProduct1.getId(),luckProduct1.getWeight().intValue());
           }
//            tt.forEach((k,v)->{
//                System.out.println(k+"   "+v);
//            });
            WeightMeta<Integer> md = RandomUtil.buildWeightMeta(tt);
            Integer id = md.random();
            List<LuckProduct> list = luckProducts.stream().filter(x -> x.getId()==id).collect(Collectors.toList());
            return list.get(0);
        }
        return  null;
    }
